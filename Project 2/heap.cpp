...
void heap::percolate_up(int current_pos) {
  ...
  while (true) {
    ...
    if (key_value < data[parent_index]) {
  ...
  str_map.setPointer(data[curent_pos].id, &(data[current_pos]));
}

////int heap::set_key(const std::string& id, int key) {
  
/////}

int heap::delete_min(std::string* p_id, int* p_key, void* pp_data) {
